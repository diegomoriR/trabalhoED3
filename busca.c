    ...
    // idadePessoa
        ...
        if(p.removido == '0'){ //registro nao esta marcado como removido
            ...
            p.nomeUsuario[p.tamanhoNomeUsuario] = 0;
                ...
                encontrado++;//aumenta o n�mero de registros encontrados
                //imprime os dados
                lista_segue(fd, fdh, fdo, p);
                free(p.nomePessoa);
                free(p.nomeUsuario);
...
void busca_substitui(FILE *fd, FILE *fdh){
  ...
  if(fd == NULL || fdh == NULL) {
   ...
   int buscas;
    ...
    // criando headers e pessoas pra facilitar a manipula��o de dados
    headerIndice hi;
    header h;
    pessoa p;
    indice *i;
        ...
        scanf(" %[^=]", tipoBusca);
        scanf("%*c");
        fseek(fd, 0, SEEK_SET);//coloca no come�o do arquivo pessoa
        ...
        printf("Registro inexistente.\n\n");
        scanf("%*c");
        scanf("%s", parametro);//l� qual campo quer substituir para funcionamento das pr�ximas chamadas
        return;}
        ...
        if(p.removido == '0'){ //registro nao esta marcado como removido
            ...
            p.nomeUsuario[p.tamanhoNomeUsuario] = 0;
                ...
                b++;//aumenta o n�mero de registros encontrados
                //imprime os dados
                substitui_registro(fd,fdh,p, offset);
                }
            free(p.nomePessoa);
    ...
    indice i;
        ...
        if(p.removido == '0'){ //registro nao esta marcado como removido
            ...
            p.nomeUsuario[p.tamanhoNomeUsuario] = 0;
                 ...
                 if(p.idadePessoa == par){//verifica se � o par�metro buscado
            ...
            }else{fseek(fd,p.tamanhoRegistro - 5,SEEK_CUR);//registro removido, vai para o pr�ximo
                free(p.nomePessoa);
                free(p.nomeUsuario);}
            }
            if(b==0){printf("Registro inexistente.\n\n");}//nenhum registro b com esse par�metro
...
int buscar_indice_por_id(Vertice *grafo, int qtd, int id) {
    for (int i = 0; i < qtd; i++) {
        if (grafo[i].idPessoa == id) return i;
    }
    return -1;
}

char* buscar_nome_por_id(Vertice *grafo, int qtd, int id) {
    int idx = buscar_indice_por_id(grafo, qtd, id);
    if (idx != -1) return grafo[idx].nomeUsuario;
    return NULL;
}
...
int buscar_indice_por_nome(Vertice *grafo, int qtd, const char* nome) {
    ...
    while (esq <= dir) {
        int meio = (esq + dir) / 2;
        int cmp = strcmp(grafo[meio].nomeUsuario, nome);
