...
void ORDER_BY(char* arquivoEntrada, char* arquivoSaida){
    ...
    for(int i =0; i<hs.quantidadePessoas;i++){//copiando os registros para o vetor
        ...
        Ps[i].dataInicioQueSegue = (char *) malloc(11);
        fread(Ps[i].dataInicioQueSegue,sizeof(char),10,fdin);
        Ps[i].dataInicioQueSegue[11]= '\0';
        Ps[i].dataFimQueSegue = (char *) malloc(11);
        fread(Ps[i].dataFimQueSegue,sizeof(char),10,fdin);
        Ps[i].dataFimQueSegue[11]= '\0';
        fread(&Ps[i].grauAmizade,sizeof(char),1,fdin);
    }
    ...
    fwrite(&Ps[i].grauAmizade,sizeof(char),1,fdout);
    offset = offset+30;
    }
